...
gandi vm create --datacenter US --memory 512 --cores 1 --ip-version 4   --hostname $VM --image "Debian 8 64 bits (HVM$

sleep 60

# Création disque data (cible de l'install de CoreOS) et attachement à la VM :

gandi disk create --name core_sys --size 10G --datacenter US -vm $VM
